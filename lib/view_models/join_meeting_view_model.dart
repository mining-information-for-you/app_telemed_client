...
class JoinMeetingViewModel extends ChangeNotifier {
  ...
  bool verifyParameters(String hashRoom) {
    if (hashRoom.isEmpty || hashRoom.isEmpty) {
      _createError(Response.empty_parameter);
      return false;
    } else if (hashRoom.length < 2 ||
        hashRoom.length > 64 ||
        hashRoom.length < 2 ||
        hashRoom.length > 64) {
  ...
  Future<bool> joinMeeting(MeetingViewModel meetingProvider,
      MethodChannelCoordinator methodChannelProvider, String hashRoom) async {
